...
export default (w, h, options) => {
    ...
    floorMesh.rotation.x = options.rotation ? options.rotation : -Math.PI;

    const floorShape = new CANNON.Box(new CANNON.Vec3(floorWidth / 2, floorHeight / 2, floorDepth / 2));
    const floorBody = new CANNON.Body({ mass: 0 });
    floorBody.addShape(floorShape);
    floorBody.quaternion.setFromAxisAngle(new CANNON.Vec3(1, 0, 0), options.angle ? options.angle : -Math.PI / 4); // Set the angle (45 degrees)
